var URLSERVER = "http://192.168.0.103/comprarador/movil/";
var URLIMAGENES = "http://192.168.0.103/comprarador/imagenes_productos/";
...
$(document).ready(
        function() {
            ...
            $('#botonSeleccionarLista').click(
                    function() {
                        if ($('#contenidoPaginaSeleccionLista div select').val() != ""
                                && $('#contenidoPaginaSeleccionLista div select').val() != "-") {
                            listaPredeterminadaAgregarProducto = $(
                                    '#contenidoPaginaSeleccionLista div select').val();
                        }

                        if (listaPredeterminadaAgregarProducto != ""
                                && listaPredeterminadaAgregarProducto != "-") {
                            agregarProducto(productoActual.id, listaPredeterminadaAgregarProducto,
                                    productoActual.marca, productoActual.descripcion,
                                    productoActual.formato);
                            history.back();
                        } else {
                            mostrarMensaje("Error",
                                    "Debe seleccionar una lista para agregar el producto");
                        }
                    });
            ...
            $('#botonFooterAgregarAFavoritos').click(
                    function() {
                        agregarFavorito(productoActual.id, productoActual.marca,
                                productoActual.descripcion, productoActual.formato);
                    });

            $('#botonFooterAgregarProductoListaAFavoritos').click(
                    function() {
                        agregarFavorito(productoActual.id, productoActual.marca,
                                productoActual.descripcion, productoActual.formato);
                    });
...
function construirPaginaBusqueda(xml) {
    ...
    if (acierto == "si") {
                ...
                .each(
                        function() {
                            ...
                            textoAagregar = "<li><a href='#' id='enlaceBusquedaProductos-" + id
                                    + "'>";
                            textoAagregar += "<img src='" + URLIMAGENES + id + ".gif' title='"
                                    + descripcion + "'/>";
                            textoAagregar += "<h3>" + marca + "</h3>";
                            textoAagregar += "<p>" + "(" + formato + ")" + descripcion + "</p></a>";
                            textoAagregar += "<a href='#' id='enlaceBusquedaProductosMenu-"
                                    + id
                                    + "'data-role='button' data-icon='gear' data-iconpos='notext' data-rel='dialog' data-transition='pop'>Menú</a>";
                            textoAagregar += "</li>";
        ...
        mostrarMensaje("Error",
                "No se han encontrado coincidencias en la base de datos de productos.");
    }
    ...
    $(xml).find('mercadona').each(
            function() {
                ...
                if (precio_mercadona != "0.00") {
                    html += "<span class='precio'> <b> " + precio_mercadona + " ("
                            + precio_relativo_mercadona + ")</b></span>";
                }

                if (oferta_mercadona != "") {
                    html += "<img id='ofertaMercadona' src='imagenes/oferta.png' alt='"
                            + oferta_mercadona + "' />";
                }
                html += "</p>";
            }); // close each(

    $(xml).find('carrefour').each(
            function() {
                ...
                if (precio_carrefour != "0.00") {
                    html += "<span class='precio'><b> " + precio_carrefour + " ("
                            + precio_relativo_carrefour + ")</b></span>";
                }

                if (oferta_carrefour != "") {
                    html += "<img id='ofertaCarrefour' src='imagenes/oferta.png' alt='"
                            + oferta_carrefour + "' />";
                }
                html += "</p>";
            }); // close each(

    $(xml).find('hipercor').each(
            function() {
                ...
                if (precio_hipercor != "0.00") {
                    html += "<span class='precio'><b> " + precio_hipercor + " ("
                            + precio_relativo_hipercor + ")</b></span>";
                }

                if (oferta_hipercor != "") {
                    html += "<img id='ofertaHipercor' src='imagenes/oferta.png' alt='"
                            + oferta_hipercor + "' />";
                }
                html += "</p>";
            }); // close each(

    $(xml).find('corteIngles').each(
            function() {
                ...
                if (precio_corteIngles != "0.00") {
                    html += "<span class='precioCorte'><b> " + precio_corteIngles + " ("
                            + precio_relativo_corteIngles + ")</b></span>";
                }

                if (oferta_corteIngles != "") {
                    html += "<img id='ofertaCorteIngles' src='imagenes/oferta.png' alt='"
                            + oferta_corteIngles + "' />";
                }
                html += "</p>";
            }); // close each(
...
function construirPaginaCategorias(xml) {
    ...
    $(xml).find('categoria').each(
            function() {
                var nombre = $(this).attr('nombre').replace('/.º/', 'u');
                var posicion = $(this).attr('pos');
                textoAagregar = "<li data-icon='arrow-r'><a href='#' id='enlaceCategoria-"
                        + posicion + "'>" + nombre + "</a></li>";
...
function construirPaginaSubcategorias(xml, nombreCategoria) {
    ...
    $(xml).find('subcategoria').each(
            function() {
                var nombre = $(this).attr('nombre').replace('/.º/', 'u');
                var posicion = $(this).attr('pos');
                textoAagregar = "<li data-icon='arrow-r'><a href='#' id='enlaceSubcategoria-"
                        + posicion + "'>" + nombre + "</a></li>";
...
function construirPaginaSubsubcategorias(xml, nombreSubcategoria) {
    ...
    $(xml).find('subsubcategoria').each(
            function() {
                var nombre = $(this).attr('nombre').replace('/.º/', 'u');
                var posicion = $(this).attr('pos');
                textoAagregar = "<li data-icon='arrow-r'><a href='#' id='enlaceSubsubcategoria-"
                        + posicion + "'>" + nombre + "</a></li>";
...
function construirPaginaProductosCategoria(xml,nombre) {
    ...
    if (acierto == "si") {
        ...
        $(xml)
                ...
                .each(
                        function() {
                            ...
                            textoAagregar += "<h3>" + marca + "</h3>";
                            textoAagregar += "<p>" + "(" + formato + ")" + descripcion + "</p></a>";
                            textoAagregar += "<a href='#' id='enlaceListaProductosMenu-"
                                    + id
                                    + "'data-role='button' data-icon='gear' data-iconpos='notext' data-rel='dialog' data-transition='pop'>Menu</a>";
                            textoAagregar += "</li>";
        ...
        $.mobile.hidePageLoadingMsg();
        mostrarMensaje("Error",
                "No ha sido posible encontrar la informaci\u00f3n relativa al producto");
    }
}
...
function comprobarLista(ids) {
    ...
    if (verificarConexion()) {
        ...
        mostrarMensaje("Error",
                "El dispositivo no est\u00e1 conectado a Internet. Verifique su conexi\u00f3n.");
...
function representarGraficaResultados(datos, tooltips) {
    ...
    hbar.Set('chart.colors', ['rgba(43,97,48,0.85)', 'rgba(2,45,124,0.85)', 'rgba(25,86,130,0.85)',
            'rgba(0,166,80,0.85)']);
...
function mostrarSubsubcategorias(pos, nombreSubcategoria) {
    ...
    if (verificarConexion()) {
        ...
        $.ajax({
            type : "GET",
            url : URLSERVER + "obtenerSubsubcategoria.php?cat=" + categoriaActual + "&subcat="
                    + pos,
            dataType : "xml",
            beforeSend : function() {
                $.mobile.showPageLoadingMsg();
...
function obtenerProductosCategoria(nombre) {
    ...
    if (verificarConexion()) {
        ...
        $.ajax({
            type : "GET",
            url : URLSERVER + "obtenerProducto.php?cat=" + categoriaActual + "&subcat="
                    + subcategoriaActual + "&subsubcat=" + subsubcategoriaActual,
            dataType : "xml",
            beforeSend : function() {
                $.mobile.showPageLoadingMsg();
            },
    ...
    try {
        ...
        bd
                ...
                function(transaction) {
                    transaction
                            .executeSql(
                                    'CREATE TABLE listas(id INTEGER NOT NULL PRIMARY KEY , nombre TEXT NOT NULL DEFAULT "");',
                                    [], function() {
                    ...
                    transaction
                            .executeSql(
                                    'CREATE TABLE favoritos(id INTEGER NOT NULL PRIMARY KEY , marca TEXT NOT NULL DEFAULT "", descripcion TEXT NOT NULL DEFAULT "", formato TEXT NOT NULL DEFAULT "");',
                                    [], function() {
    ...
    try {
        bd.transaction(function(transaction) {
            if (opcion == LISTAS_MOSTRADO) { // modo mostrado de listas
                ...
                transaction.executeSql('SELECT * FROM listas ORDER BY id', [],
                        buscarListasHandler1, errorHandler);
                ...
                transaction.executeSql('SELECT * FROM listas ORDER BY id', [],
                        buscarListasHandler2, errorHandler);
                transaction.executeSql('SELECT * FROM listas ORDER BY id', [],
                        buscarListasHandler2, errorHandler);
            }
        });
...
function buscarListasHandler1(transaction, results) {
    ...
    for ( var i = 0; i < results.rows.length; i++) {
        ...
        html = "<li data-icon=arrow-r><a href='#' id='enlaceLista-"
                + id
                + "'>"
                + nombre
                + "</a><a href='#' id='enlaceListaMenu-"
                + id
...
function buscarListasHandler2(transaction, results) {
    ...
    for ( var i = 0; i < results.rows.length; i++) {
        ...
        html = "<option value='" + id + "' id='opcionSeleccionLista-" + id + "'>" + nombre
                + "</option>";
    ...
    try {
        bd.transaction(function(transaction) {
            if (opcion == PRODUCTOS_MOSTRAR) { // modo mostrado de productos de
                // una lista
                transaction.executeSql('SELECT * FROM productos WHERE lista="' + idLista + '";',
                        [], buscarProductosListaHandler1, errorHandler);
            } else if (opcion == PRODUCTOS_COMPROBAR) { // modo comprobar
                transaction.executeSql('SELECT * FROM productos WHERE lista="' + idLista + '";',
                        [], buscarProductosListaHandler2, errorHandler);
            } else if (opcion == PRODUCTOS_COMPRA) { // modo compra
                nombreListaModoCompraActual = nombre + " (modo compra)";
                transaction.executeSql('SELECT * FROM productos WHERE lista="' + idLista + '";',
                        [], buscarProductosListaHandler3, errorHandler);
            }
...
function buscarProductosListaHandler1(transaction, results) {
    ...
    for ( var i = 0; i < results.rows.length; i++) {
        ...
        html += "<a href='#' id='enlaceProductoListaMenu-"
                + id
                + "' data-role='button' data-icon='gear' data-iconpos='notext' data-rel='dialog' data-transition='pop'>Menú</a>";
        html += "</li>";
...
function buscarFavoritos() {
    try {
        bd.transaction(function(transaction) {
            transaction.executeSql('SELECT * FROM favoritos', [], buscarFavoritosHandler,
                    errorHandler);
        });
    } catch (e) {
...
function buscarFavoritosHandler(transaction, results) {
    ...
    for ( var i = 0; i < results.rows.length; i++) {
        ...
        html += "<h3>" + marca + "</h3>";
        html += "<p>(" + formato + ") " + descripcion + "</p></a>";
        html += "<a href='#' id='enlaceListaFavoritosMenu-"
                + id
                + "' data-role='button' data-icon='gear' data-iconpos='notext' data-rel='dialog' data-transition='pop'>Menú</a>";
        html += "</li>";
    ...
    try {
        bd.transaction(function(transaction) {
            ...
            transaction.executeSql(
                    'replace into favoritos (id,marca,descripcion,formato) VALUES ("' + id + '","'
                            + marca + '","' + descripcion + '","' + formato + '");', [],
                    function() {
...
function agregarProducto(id, lista, marca, descripcion, formato) {
    ...
    try {
        bd.transaction(function(transaction) {
            ...
            // existe, actualiza sus valores. Si no existe, agrega el producto
            transaction.executeSql(
                    'replace into productos (id,lista,marca,descripcion,formato) VALUES ("' + id
                            + '","' + lista + '","' + marca + '","' + descripcion + '","' + formato
                            + '");', [], function() {
...
function iniciarAcelerometro() {
    ...
    acelerometroID = navigator.accelerometer.watchAcceleration(exitoAcelerometro,
            errorAcelerometro, options);
}
                        ...
                        if (codigo.format == "EAN_13") {
                            ...
                            if (verificarConexion()) {
                                $
                                        .ajax({
                                            type : "GET",
                                            url : URLSERVER + "obtenerProducto.php?codigo="
                                                    + codigo.text,
                                            dataType : "xml",
                                            beforeSend : function() {
                                                $.mobile.showPageLoadingMsg();
                                            ...
                                            success : function(xml) {
                                                ...
                                                if (acierto == "si") {
                                                    $(xml)
                                                            ...
                                                            .each(
                                                                    function() {
                                                                        ...
                                                                        // y se
                                                                        // muestra
                                                                        eventoProductoListaCategoriaPulsado(
                                                                                id, marca,
                                                                                descripcion,
                                                                                formato);
                                                                        vistaProducto = true;
                                                    ...
                                                    mostrarMensaje("Error",
                                                            "El c\u00f3digo de barras no se encuentra en la base de datos.")
                                                }
                            ...
                            mostrarMensaje("Error",
                                    "El c\u00f3digo leido no es un c\u00f3digo de barras tipo EAN 13.")
                        }
                    }, function(error) {
