var URLSERVER = "http://192.168.0.105/comprarador/movil/";
var URLIMAGENES = "http://192.168.0.105/comprarador/imagenes_productos/";
...
$(document).ready(function() {
	...
	// se establece el texto que se muestra en la zona de inserción de
	...
	$("#botonCodigoBarras").click(function() {
		console.log("BOTON CODIGO BARRAS PULSADO");
	...
	$('#botonSeleccionarLista').click(function() {
		...
		if (listaPredeterminadaAgregarProducto != "" && listaPredeterminadaAgregarProducto != "-") {
			tiempoInicioAgregarProducto = Date.now();
	...
	// EVENTOS CAMBIO PÁGINA DESDE PÁGINA VISUALIZACIÓN DE PRODUCTO.
	// si se sale de una página de vista de producto se desactiva la
	...
	// /////////////////////////////////////////////
	// EVENTOS MEDICI‚àö√¨N DE TIEMPOS DE RESPUESTA
	$('#paginaListas').live('pageshow', function(event, ui) {
		tiempoFinAccesoVistaListas = Date.now();
		console.log("  - Tiempo (ms) acceso vista listas: " + (tiempoFinAccesoVistaListas - tiempoInicioAccesoVistaListas));
	});

	$('#paginaLista').live('pageshow', function(event, ui) {
		tiempoFinAccesoLista = Date.now();
		console.log("  - Tiempo (ms) acceso lista: " + (tiempoFinAccesoLista - tiempoInicioAccesoLista));
	});

	$('#paginaResultados').live('pageshow', function(event, ui) {
		tiempoFinComprobarLista = Date.now();
		console.log("  - Tiempo (ms) comprobar lista: " + (tiempoFinComprobarLista - tiempoInicioComprobarLista));
	});

	$('#paginaFavoritos').live('pageshow', function(event, ui) {
		tiempoFinAccesoVistaFavoritos = Date.now();
		console.log("  - Tiempo (ms) acceso vista favoritos: " + (tiempoFinAccesoVistaFavoritos - tiempoInicioAccesoVistaFavoritos));
	});

	$('#paginaListaCategorias').live('pageshow', function(event, ui) {
		tiempoFinMostrarCategorias = Date.now();
		console.log("  - Tiempo (ms) mostrar categorias: " + (tiempoFinMostrarCategorias - tiempoInicioMostrarCategorias));
	});

	$('#paginaListaSubcategorias').live('pageshow', function(event, ui) {
		tiempoFinMostrarSubcategorias = Date.now();
		console.log("  - Tiempo (ms) mostrar subcategorias: " + (tiempoFinMostrarSubcategorias - tiempoInicioMostrarSubcategorias));
	});

	$('#paginaListaSubsubcategorias').live('pageshow', function(event, ui) {
		tiempoFinMostrarSubsubcategorias = Date.now();
		console.log("  - Tiempo (ms) mostrar subsubcategorias: " + (tiempoFinMostrarSubsubcategorias - tiempoInicioMostrarSubsubcategorias));
	});

	$('#paginaListaProductos').live('pageshow', function(event, ui) {
		tiempoFinMostrarProductosCategoria = Date.now();
		console.log("  - Tiempo (ms) mostrar productos categoria: " + (tiempoFinMostrarProductosCategoria - tiempoInicioMostrarProductosCategoria));
	});

	$('#paginaProducto').live('pageshow', function(event, ui) {

		if (typeof (window['tiempoInicioAccesoProducto']) != "undefined") {
			tiempoFinAccesoProducto = Date.now();
			console.log("  - Tiempo (ms) acceso producto: " + (tiempoFinAccesoProducto - tiempoInicioAccesoProducto));
		}

		if (typeof (window['tiempoInicioCodigoBarras']) != "undefined") {
			tiempoFinCodigoBarras = Date.now();
			console.log("  - Tiempo (ms) b√î√∏Œ©squeda c√î√∏Œ©digo barras: " + (tiempoFinCodigoBarras - tiempoInicioCodigoBarras));
		}
...
function construirPaginaResultados(ids, xml) {
	...
	$(xml).find('mercadona').each(function() {
		var totalMercadona = $(this).attr('total');
		if (totalMercadona == "") {
			totalMercadona = 0;
		}
		var aciertosMercadona = $(this).attr('aciertos');
		datos.push(parseFloat(totalMercadona));
		aciertos.push(aciertosMercadona + "/" + ids.length);
	}); // close each(

	$(xml).find('carrefour').each(function() {
		var totalCarrefour = $(this).attr('total');
		if (totalCarrefour == "") {
			totalCarrefour = 0;
		}
		var aciertosCarrefour = $(this).attr('aciertos');
		datos.push(parseFloat(totalCarrefour));
		aciertos.push(aciertosCarrefour + "/" + ids.length);
	}); // close each(

	$(xml).find('hipercor').each(function() {
		var totalHipercor = $(this).attr('total');
		if (totalHipercor == "") {
			totalHipercor = 0;
		}
		var aciertosHipercor = $(this).attr('aciertos');
		datos.push(parseFloat(totalHipercor));
		aciertos.push(aciertosHipercor + "/" + ids.length);

	}); // close each(

	$(xml).find('corteIngles').each(function() {
		var totalCorteIngles = $(this).attr('total');
		if (totalCorteIngles == "") {
			totalCorteIngles = 0;
		}
		var aciertosCorteIngles = $(this).attr('aciertos');
...
function construirPaginaBusqueda(xml) {
	...
	if (acierto == "si") {
		...
		tiempoFinBusqueda = Date.now();
		console.log("  - Tiempo (ms) busqueda: " + (tiempoFinBusqueda - tiempoInicioBusqueda));
	} else {
...
function construirPaginaProductoCategoria(xml, html) {
	...
	$(xml).find('mercadona').each(function() {
		var precio_mercadona = $(this).attr('precio');
		var precio_relativo_mercadona = $(this).attr('relativo');
		var oferta_mercadona = $(this).attr('oferta');
	...
	$(xml).find('carrefour').each(function() {
		var precio_carrefour = $(this).attr('precio');
		var precio_relativo_carrefour = $(this).attr('relativo');
		var oferta_carrefour = $(this).attr('oferta');
	...
	$(xml).find('hipercor').each(function() {
		var precio_hipercor = $(this).attr('precio');
		var precio_relativo_hipercor = $(this).attr('relativo');
		var oferta_hipercor = $(this).attr('oferta');
	...
	$(xml).find('corteIngles').each(function() {
		var precio_corteIngles = $(this).attr('precio');
		var precio_relativo_corteIngles = $(this).attr('relativo');
		var oferta_corteIngles = $(this).attr('oferta');
...
function construirPaginaCategorias(xml) {
	...
	$(xml).find('categoria').each(function() {
		var nombre = $(this).attr('nombre').replace('/.º/', 'u');
...
function construirPaginaSubsubcategorias(xml, nombreSubcategoria) {
	...
	$(xml).find('subsubcategoria').each(function() {
		var nombre = $(this).attr('nombre').replace('/.º/', 'u');
		var posicion = $(this).attr('pos');
...
function construirPaginaProductosCategoria(xml, nombre) {
	...
	if (acierto == "si") {
		...
		$.mobile.hidePageLoadingMsg();
		mostrarMensaje("Error", "No ha sido posible encontrar la informaci\u00f3n relativa al producto");
	}
}
...
function comprobarLista(ids) {
	...
	if (verificarConexion()) {
		...
		$.ajax({
			type : "GET",
			url : URLSERVER + "comprobar.php?" + params,
			dataType : "xml",
			beforeSend : function() {
				...
				'Eliminar' : {
					click : function() {
						if (confirm("Desea eliminar el producto de la lista?")) {
							...
							buscarListas(LISTAS_MOSTRADO);
							// se hace click sobre la lista que se estaba viendo
...
function eventoBuscarProductoPulsado(texto) {
	...
	if (verificarConexion()) {
		...
		tiempoInicioBusqueda = Date.now();

		$.ajax({
			type : "GET",
			url : URLSERVER + "obtenerProducto.php?texto=" + texto,
			dataType : "xml",
			beforeSend : function() {
				$.mobile.showPageLoadingMsg();
			},
...
function eventoListaPulsada(id, nombre) {
	...
	tiempoInicioAccesoLista = Date.now();
...
function eventoProductoListaCategoriaPulsado(id, marca, descripcion, formato) {
	...
	tiempoInicioAccesoProducto = Date.now();
	...
	if (verificarConexion()) {
		...
		$.ajax({
			type : "GET",
			url : URLSERVER + "obtenerPrecios.php?id=" + id,
			dataType : "xml",
			beforeSend : function() {
				$.mobile.showPageLoadingMsg();
			},
...
function mostrarCategorias() {
	...
	if (verificarConexion()) {
		...
		tiempoInicioMostrarCategorias = Date.now();
		console.log("  - Inicio mostrar categorias: " + tiempoInicioMostrarCategorias);

		$.ajax({
			type : "GET",
			url : URLSERVER + "obtenerMenu.php",
			dataType : "xml",
			beforeSend : function() {
				$.mobile.showPageLoadingMsg();
			},
...
function mostrarSubcategorias(pos, nombreCategoria) {
	...
	if (verificarConexion()) {
		...
		tiempoInicioMostrarSubcategorias = Date.now();

		// se obtienen las subcategorías correspondientes a la categoría pulsada
		$.ajax({
			type : "GET",
			url : URLSERVER + "obtenerSubcategoria.php?cat=" + pos,
			dataType : "xml",
			beforeSend : function() {
				$.mobile.showPageLoadingMsg();
			},
...
function mostrarSubsubcategorias(pos, nombreSubcategoria) {
	...
	if (verificarConexion()) {
		...
		tiempoInicioMostrarSubsubcategorias = Date.now();

		// se obtienen las subcategorías correspondientes a la categoría pulsada
		$.ajax({
			type : "GET",
			url : URLSERVER + "obtenerSubsubcategoria.php?cat=" + categoriaActual + "&subcat=" + pos,
			dataType : "xml",
			beforeSend : function() {
				$.mobile.showPageLoadingMsg();
			},
			...
			success : function(xml) {
				...
				if ($(xml).find('subsubcategoria').length) {
...
function obtenerProductosCategoria(nombre) {
	...
	if (verificarConexion()) {
		...
		tiempoInicioMostrarProductosCategoria = Date.now();
		console.log("  - Inicio mostrar productos categoria: " + tiempoInicioMostrarProductosCategoria);

		$.ajax({
			type : "GET",
			url : URLSERVER + "obtenerProducto.php?cat=" + categoriaActual + "&subcat=" + subcategoriaActual + "&subsubcat=" + subsubcategoriaActual,
			dataType : "xml",
			beforeSend : function() {
				$.mobile.showPageLoadingMsg();
			},
...
// /////////////////////////////////////////////////////////////////////////
// VACIAR BD: se eliminan todas las filas de las tablas
// /////////////////////////////////////////////////////////////////////////
function vaciarBD() {
	try {
		bd.transaction(function(transaction) {
			transaction.executeSql('DROP TABLE IF EXISTS listas;', [], function() {
			}, errorHandler);
			transaction.executeSql('DROP TABLE IF EXISTS productos;', [], function() {
			}, errorHandler);
			transaction.executeSql('DROP TABLE IF EXISTS favoritos;', [], function() {
			}, errorHandler);
		});
	} catch (e) {
...
function crearTablas() {
	...
	try {
		...
		function(transaction) {
			...
			transaction.executeSql('insert into favoritos (id,marca,descripcion,formato) VALUES ("1","MarcaFav1","DescripFav1","FormFav1");', [], function() {
			}, errorHandler);
			transaction.executeSql('insert into favoritos (id,marca,descripcion,formato) VALUES ("2","MarcaFav2","DescripFav2","FormFav2");', [], function() {
			}, errorHandler);
			transaction.executeSql('insert into favoritos (id,marca,descripcion,formato) VALUES ("3","MarcaFav3","DescripFav3","FormFav3");', [], function() {
			}, errorHandler);
			transaction.executeSql('insert into favoritos (id,marca,descripcion,formato) VALUES ("4","MarcaFav4","DescripFav4","FormFav4");', [], function() {
			}, errorHandler);
//			transaction.executeSql('insert into favoritos (id,marca,descripcion,formato) VALUES ("5","MarcaFav5","DescripFav5","FormFav5");', [], function() {
//			}, errorHandler);
//			transaction.executeSql('insert into favoritos (id,marca,descripcion,formato) VALUES ("6","MarcaFav1","DescripFav1","FormFav1");', [], function() {
//			}, errorHandler);
//			transaction.executeSql('insert into favoritos (id,marca,descripcion,formato) VALUES ("7","MarcaFav2","DescripFav2","FormFav2");', [], function() {
//			}, errorHandler);
//			transaction.executeSql('insert into favoritos (id,marca,descripcion,formato) VALUES ("8","MarcaFav3","DescripFav3","FormFav3");', [], function() {
//			}, errorHandler);
//			transaction.executeSql('insert into favoritos (id,marca,descripcion,formato) VALUES ("9","MarcaFav4","DescripFav4","FormFav4");', [], function() {
//			}, errorHandler);
//			transaction.executeSql('insert into favoritos (id,marca,descripcion,formato) VALUES ("10","MarcaFav5","DescripFav5","FormFav5");', [], function() {
//			}, errorHandler);

			transaction.executeSql('insert into productos (id,lista,marca,descripcion,formato) VALUES ("1","1","MarcaProd1","DescripProd1","FormProd1");', [], function() {
			}, errorHandler);
			transaction.executeSql('insert into productos (id,lista,marca,descripcion,formato) VALUES ("2","1","MarcaProd2","DescripProd2","FormProd2");', [], function() {
			}, errorHandler);
			transaction.executeSql('insert into productos (id,lista,marca,descripcion,formato) VALUES ("3","1","MarcaProd3","DescripProd3","FormProd3");', [], function() {
			}, errorHandler);
			transaction.executeSql('insert into productos (id,lista,marca,descripcion,formato) VALUES ("4","1","MarcaProd4","DescripProd4","FormProd4");', [], function() {
			}, errorHandler);
			transaction.executeSql('insert into productos (id,lista,marca,descripcion,formato) VALUES ("5","1","MarcaProd5","DescripProd5","FormProd5");', [], function() {
			}, errorHandler);

			transaction.executeSql('insert into productos (id,lista,marca,descripcion,formato) VALUES ("6","2","MarcaProd6","DescripProd6","FormProd6");', [], function() {
			}, errorHandler);
			transaction.executeSql('insert into productos (id,lista,marca,descripcion,formato) VALUES ("7","2","MarcaProd7","DescripProd7","FormProd7");', [], function() {
			}, errorHandler);
			transaction.executeSql('insert into productos (id,lista,marca,descripcion,formato) VALUES ("8","2","MarcaProd8","DescripProd8","FormProd8");', [], function() {
			}, errorHandler);
			transaction.executeSql('insert into productos (id,lista,marca,descripcion,formato) VALUES ("9","2","MarcaProd9","DescripProd9","FormProd9");', [], function() {
			}, errorHandler);
			transaction.executeSql('insert into productos (id,lista,marca,descripcion,formato) VALUES ("10","2","MarcaProd10","DescripProd10","FormProd10");', [], function() {
			}, errorHandler);
			transaction.executeSql('insert into productos (id,lista,marca,descripcion,formato) VALUES ("11","2","MarcaProd11","DescripProd11","FormProd11");', [], function() {
			}, errorHandler);
			transaction.executeSql('insert into productos (id,lista,marca,descripcion,formato) VALUES ("12","2","MarcaProd12","DescripProd12","FormProd12");', [], function() {
			}, errorHandler);
			transaction.executeSql('insert into productos (id,lista,marca,descripcion,formato) VALUES ("13","2","MarcaProd13","DescripProd13","FormProd13");', [], function() {
			}, errorHandler);
			transaction.executeSql('insert into productos (id,lista,marca,descripcion,formato) VALUES ("14","2","MarcaProd14","DescripProd14","FormProd14");', [], function() {
			}, errorHandler);
			transaction.executeSql('insert into productos (id,lista,marca,descripcion,formato) VALUES ("15","2","MarcaProd15","DescripProd15","FormProd15");', [], function() {
			}, errorHandler);

			transaction.executeSql('insert into listas (id,nombre) VALUES (NULL,"Lista");', [], function() {
			}, errorHandler);
			transaction.executeSql('insert into listas (id,nombre) VALUES (NULL,"Lista2");', [], function() {
			}, errorHandler);
			transaction.executeSql('insert into listas (id,nombre) VALUES (NULL,"Lista3");', [], function() {
			}, errorHandler);
			transaction.executeSql('insert into listas (id,nombre) VALUES (NULL,"Lista4");', [], function() {
			}, errorHandler);
			transaction.executeSql('insert into listas (id,nombre) VALUES (NULL,"Lista5");', [], function() {
			}, errorHandler);
		});
	} catch (e) {
	...
	try {
		bd.transaction(function(transaction) {
			if (opcion == PRODUCTOS_MOSTRAR) { // modo mostrado de productos de
				...
				tiempoInicioComprobarLista = Date.now();
...
function agregarFavorito(id, marca, descripcion, formato) {
	...
	tiempoInicioAgregarFavorito = Date.now();
	console.log("  - Inicio agregar favorito: " + tiempoInicioAgregarFavorito);
	...
	try {
		bd.transaction(function(transaction) {
			...
			transaction.executeSql('replace into favoritos (id,marca,descripcion,formato) VALUES ("' + id + '","' + marca + '","' + descripcion + '","' + formato + '");', [], function() {
				tiempoFinAgregarFavorito = Date.now();
				console.log("  - Tiempo (ms) agregar favorito: " + (tiempoFinAgregarFavorito - tiempoInicioAgregarFavorito));
	...
	try {
		bd.transaction(function(transaction) {
			...
			transaction.executeSql('replace into productos (id,lista,marca,descripcion,formato) VALUES ("' + id + '","' + lista + '","' + marca + '","' + descripcion + '","' + formato + '");', [],
					function() {
						tiempoFinAgregarProducto = Date.now();
						console.log("  - Tiempo (ms) agregar producto a lista: " + (tiempoFinAgregarProducto - tiempoInicioAgregarProducto));
...
function eliminarLista(id) {
	...
	tiempoInicioEliminarLista = Date.now();
	...
	try {
		bd.transaction(function(transaction) {
			...
			transaction.executeSql(sql2, [], function() {
				tiempoFinEliminarLista = Date.now();
				console.log("  - Tiempo (ms) eliminar lista: " + (tiempoFinEliminarLista - tiempoInicioEliminarLista));
...
function eliminarFavorito(id) {
	...
	tiempoInicioEliminarFavorito = Date.now();
	
	sql = "DELETE FROM favoritos WHERE id =" + id + ";";
	try {
		bd.transaction(function(transaction) {
			transaction.executeSql(sql, [], function() {
				tiempoFinEliminarFavorito = Date.now();
				console.log("  - Tiempo (ms) eliminar favorito: " + (tiempoFinEliminarFavorito - tiempoInicioEliminarFavorito));
...
function escanearCodigoBarras() {
	...
	window.plugins.barcodeScanner.scan(function(codigo) {
		...
		if (codigo.format == "EAN_13") {
			...
			tiempoInicioCodigoBarras = Date.now();
			
			// se comprueba que se tiene conexión
			if (verificarConexion()) {
				$.ajax({
					type : "GET",
					url : URLSERVER + "obtenerProducto.php?codigo=" + codigo.text,
					dataType : "xml",
					beforeSend : function() {
						$.mobile.showPageLoadingMsg();
					},
