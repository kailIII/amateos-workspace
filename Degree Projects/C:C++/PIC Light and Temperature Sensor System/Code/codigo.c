...
BOOLEAN flagLuz=true;              //indica si hay que realizar la conversión del valor de la luz
BOOLEAN flagTemperatura=true;      //indica si hay que obtener la temperatura del DS1624
int contadorSegundos=0;             //cuenta el número de segundos transcurridos (valores entre 0 y 3)   
int contadorDesborde=0;             //cuenta cuántas veces se ha desbordado el timer 1(se desborda cada 0,5 segundos)
...
unsigned int medirLuz(){  // Convierte el valor de tensión procedente del LDR
...
void main()
{
   ...
   //Configuración puertos B y C como salida
   set_tris_b(0x00);  
   set_tris_c(0x00);
