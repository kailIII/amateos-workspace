...
public class AsyncPortScan extends AsyncTask<Void, Object, Void> {
        ...
        try {
            ...
            if (nb_port > step) {
                for (int i = port_start; i <= port_end - step; i += step + 1) {
                    Log.d(getClass().getSimpleName(), "for" + i);
                    if (select) {
                        Log.d(getClass().getSimpleName(), "if " + i);
    ...
    private void start(final InetAddress ina, final int PORT_START, final int PORT_END) {
        Log.d(getClass().getSimpleName(), "PORT_START: " + PORT_START + " PORT_END: " + PORT_END);
        select = true;
        try {
            ...
            while (select && selector.keys().size() > 0) {
                if (selector.select(TIMEOUT_SELECT) > 0) { // Al menos un
                                                           // channel tiene que
                                                           // estar ready
                    synchronized (selector.selectedKeys()) {
                        ...
                        while (iterator.hasNext()) {
                            ...
                            try {
                                ...
                                // States
                                final Data data = (Data) key.attachment();// Data
                                                                          // asociado
                                                                          // al
                                                                          // socket
                                                                          // (Data{puerto,estado...})
                                ...
                                if (key.isConnectable()) {
                                    ...
                                    try {
                                        byteBuffer.clear();
                                        final int numRead = ((SocketChannel)
                                                key.channel())
                                                        .read(byteBuffer);
                                        if (numRead > 0) {
                                            String banner = new
