...
import org.inftel.socialwind.client.mobile.utils.Setup;
...
public class AccountsActivity extends Activity {
                                ...
                                public void run(AccountManagerFuture<Bundle> future) {
                                    ...
                                    mgr.getAuthToken(account, "ah", null, activity,
                                            new AccountManagerCallback<Bundle>() {
                                                public void run(AccountManagerFuture<Bundle> future) {
                                                    String authToken = getAuthToken(future);
                                                    // Convert the token into a cookie for future
                                                    // use
                                                    ...
                                                    editor.commit();
                                                    C2DMessaging
                                                            .register(mContext, Setup.SENDER_ID);
                                                }
                                            }, null);
                                }
                            }, null);
    ...
    /**
     * Retrieves the authorization cookie associated with the given token. This method should only
     * be used when running against a production appengine backend (as opposed to a dev mode
     * server).
     */
    ...
    /**
     * Returns a list of registered Google account names. If no Google accounts are registered on
     * the device, a zero-length list is returned.
     */
