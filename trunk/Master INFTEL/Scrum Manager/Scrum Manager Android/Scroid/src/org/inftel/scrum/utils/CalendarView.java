...
public class CalendarView extends ImageView {
	...
	private Cell[] mReunionCell = null;
	private Cell[][] mCells = new Cell[6][7];
	private OnCellTouchListener mOnCellTouchListener = null;
	MonthDisplayHelper mHelper;
	Drawable mDecoration = null;
	Drawable[] mDecorationReunion = null;
	boolean[][] reuniones;
	int mMesCurso = 0;
	int[] mDiaReuniones;
	...
	public CalendarView(Context context, AttributeSet attrs, int defStyle) {
		...
		mDecorationReunion = new Drawable[30];
		
		for (int i = 0; i < mDecorationReunion.length; i++) {
			mDecorationReunion[i] = context.getResources().getDrawable(R.drawable.typeb_calendar_reunion);
		}

		reuniones = new boolean[32][13];
	...
	private void initCalendarView() {
		...
		Log.d(TAG,"mMesCurso: "+mMesCurso);
		mMesCurso = getMonth() + 1;
		Log.d(TAG,"mMesCurso: "+mMesCurso);
	...
	private void initCells() {
		...
		for (int week = 0; week < mCells.length; week++) {
			for (int day = 0; day < mCells[week].length; day++) {
				...
				Log.d(TAG,"mMesCurso:"+mMesCurso);
				mDiaReuniones = getReunionesMes(mMesCurso);

				for (int d = 0; d < mDiaReuniones.length; d++) {

					if (mDiaReuniones[d] != -1 && tmp[week][day].thisMonth
							&& tmp[week][day].day == d) {
						
						mReunionCell = new Cell[mDiaReuniones.length];
						Log.d(TAG, "mes: " + mMesCurso);
						mReunionCell[d] = mCells[week][day];
						Log.d(TAG, "dia de reunion " + d);
						mDecorationReunion[d].setBounds(mReunionCell[d].getBound());
	...
	public void addReuniones(int dia, int mes) {

		Log.d(TAG, "entrando en addReuniones ,dia: " + dia);
		Log.d(TAG, "entrando en addReuniones ,mes: " + mes);

		reuniones[dia][mes] = true;

	}

	public int getDiaReuniones(int dia, int mes) {

		if (reuniones[dia][mes])
			return dia;
		else
			return -1;
	...
	public int[] getReunionesMes(int mes) {

		int[] diasReunion = new int[32];

		for (int d = 0; d < diasReunion.length; d++)
			diasReunion[d] = getDiaReuniones(d, mes);
	...
	public void nextMonth() {
		mHelper.nextMonth();
		mMesCurso = mMesCurso + 1;
		Log.d(TAG, "mMesSiguiente en nextMonth: " + mMesCurso);
		
		if(mMesCurso==13)
			mMesCurso=1;
		borrarReunionesCalendario();
		initCells();
		invalidate();
	}
	...
	public void previousMonth() {
		...
		mMesCurso = mMesCurso - 1;
		Log.d(TAG, "mMesAnterior en previousMonth: " + mMesCurso);
	...
	protected void onDraw(Canvas canvas) {
		...
		if (mDecorationReunion != null && mReunionCell != null) {

			for (Drawable m : mDecorationReunion) {
	...
	public void borrarReunionesCalendario(){
		
		for (int i = 0; i < mDecorationReunion.length; i++) {
			mDecorationReunion[i] = this.getResources().getDrawable(
					R.drawable.typeb_calendar_reunion);
		}
	}
